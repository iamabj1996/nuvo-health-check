...
const UpgradeConflict = () => {
    ...
    const upgradeConflicts = useMemo(() => {
        if (!detailedUpgradeConflictLogs || detailedUpgradeConflictLogs.length === 0) {
    ...
    const getConflictCount = (history) => {
        try {
            ...
            if (Array.isArray(parsedHistory) && parsedHistory[0] && parsedHistory[0].message === "No conflicts have been found.") {
                return 0;
            }
    ...
    useEffect(() => {
        if (upgradeConflicts) {
            setNuvoloTablesCount(upgradeConflicts?.nuvoloTablesCount.split('@')[0] || 0);
        }
    }, [upgradeConflicts]);
            ...
            return (
                <div className="mt-4">
                        ...
                        <table className="min-w-full bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700">
                            <thead>
                                <tr className="bg-gray-100 dark:bg-gray-700">
                                    {['File Name', 'Priority', 'Comments', 'Disposition', 'Type', 'Plugin', 'Table', 'Target Name'].map((header) => (
                            ...
                            <tbody>
                                {paginatedData.map((conflict, index) => (
                                    <tr key={index} className={index % 2 === 0 ? 'bg-gray-50 dark:bg-gray-600' : 'bg-white dark:bg-gray-700'}>
                                        {['fileName', 'priority', 'comments', 'disposition', 'type', 'plugin', 'table', 'targetName'].map((key) => (
                                            <td key={key} className="px-4 py-2 text-gray-800 dark:text-gray-200">{conflict[key]}</td>
                                        ))}
                                    </tr>
                                ))}
                            </tbody>
                    ...
                    <div className="mt-4 flex justify-between items-center">
                        <button
                            onClick={() => setCurrentPage(prev => ({ ...prev, [pageKey]: Math.max(1, prev[pageKey] - 1) }))}
                            disabled={currentPage[pageKey] === 1}
                            className="px-4 py-2 bg-sky-500 text-white rounded disabled:bg-gray-300"
                        >
                            Previous
                        </button>
                        <span>Page {currentPage[pageKey]} of {totalPages}</span>
                        <button
                            onClick={() => setCurrentPage(prev => ({ ...prev, [pageKey]: Math.min(totalPages, prev[pageKey] + 1) }))}
    ...
    if (!upgradeConflicts) {
        return <div className="text-center text-gray-600 dark:text-gray-400">No upgrade conflict data available.</div>;
    }

    return (
        <div className="p-4 bg-white dark:bg-gray-800 rounded-lg shadow">
            {nuvoloTablesCount > 0 && (
                <p className="mb-4 text-gray-600 dark:text-gray-400">Nuvolo's Table Count: {nuvoloTablesCount}</p>
            )}

            {/* Cards for both P1 and P2 */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
                {renderCard('Upgrade History P1', getConflictCount(upgradeConflicts.upgradeHistoryP1))}
                {renderCard('Upgrade History P2', getConflictCount(upgradeConflicts.upgradeHistoryP2))}
            </div>

            <div className="flex border-b border-gray-300 mb-4">
                <button
                    onClick={() => setActiveTab('P1')}
                    className={`flex-1 py-2 px-4 text-center focus:outline-none ${activeTab === 'P1' ? 'bg-sky-500 text-white' : 'text-white'}`}
                >
                    Upgrade History P1
                </button>
                <button
                    onClick={() => setActiveTab('P2')}
                    className={`flex-1 py-2 px-4 text-center focus:outline-none ${activeTab === 'P2' ? 'bg-sky-500 text-white' : 'text-white'}`}
                >
                    Upgrade History P2
                </button>
            </div>
            ...
            {activeTab === 'P1' && renderConflictTable(upgradeConflicts.upgradeHistoryP1, "Upgrade History P1", "P1")}
            {activeTab === 'P2' && renderConflictTable(upgradeConflicts.upgradeHistoryP2, "Upgrade History P2", "P2")}
        </div>
    );
};
