  ...
  useEffect(() => {
    ...
    const fetchApplicationData = async () => {
      try {
        ...
        const response = await axios.get(
          "/api/now/table/x_nuvo_health_scan_probe_type?sysparm_display_value=true&sysparm_fields=name%2Csys_id&sysparm_limit=100"
        );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (
        ...
        const utcDateTime = moment(selectedDateTime)
          .utc()
    ...
    <div className="min-h-screen bg-gray-900 text-gray-100">
      ...
      {errorMessage && (
        <Toast
          ...
          error={
            errorMessage.startsWith("Health Check initiated successfully")
              ? false
              : true
          }
        />
