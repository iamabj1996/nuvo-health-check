...
const ChartsAndTableComponent = () => {
    ...
    const updatePieCharts = (healthCheckData) => {
        const scriptTypeCount = healthCheckData.New.concat(healthCheckData.Modified).reduce((acc, entry) => {
        ...
        const newPieChartData = Object.entries(scriptTypeCount).map(([name, value]) => ({ name, value }));
        setPieChartData(newPieChartData);

        const differencesScriptTypeCount = healthCheckData.Differences.reduce((acc, entry) => {
            const scriptType = entry.script_type;
            if (scriptType) {
                acc[scriptType] = (acc[scriptType] || 0) + 1;
            }
        ...
        const newDifferencesPieChartData = Object.entries(differencesScriptTypeCount).map(([name, value]) => ({ name, value }));
        setPieChartDifferencesData(newDifferencesPieChartData);
    };

    useEffect(() => {
        const barChartData = parsedData.slice(-healthCheckCount).map((entry, index) => ({
            index: parsedData.length - healthCheckCount + index,
            name: 'Health Check ' + (parsedData.length - healthCheckCount + index + 1),
            ...
            series: [
                {
                    argumentField: 'name',
                    valueField: 'value',
                    // label: { visible: true, connector: { visible: true, width: 1 } },
                    label: {
                        visible: false,
                    },
                },
            ...
            legend: {
                ...
                customizeItems(items) {
                    return items.map(function (item) {
                        var value = item && item.points && item.points[0] && item.points[0].data && item.points[0].data.value;
                        return Object.assign({}, item, {
                            text: '(' + value + ') ' + item.argument
                        });
                    });
                }
        ...
        const barChart = new dxChart(barChartRef.current, {
            ...
            series: [
                { valueField: 'newEntries', name: 'New', color: '#4caf50', stack: 'stack1' },
                { valueField: 'modifiedEntries', name: 'Modified', color: '#f44336', stack: 'stack1' },
                { valueField: 'differencesEntries', name: 'Differences', type: 'line', color: 'yellow', valueAxis: 'axis1' },
            ],
            ...
            onPointClick: (e) => {
                ...
                if (healthCheckData) {
                    ...
                    setSelectedData([...healthCheckData.New, ...healthCheckData.Modified]);
                    setSelectedDifferencesData(healthCheckData.Differences.length > 0 ? [...healthCheckData.Differences] : []);
                } else {
                    setPieChartData([]);
                    setPieChartDifferencesData([]);
                    setSelectedHealthCheckNumber(0);
                    setSelectedData([]);
                    setSelectedDifferencesData([]);
                }
            },
    ...
    }, [parsedData, pieChartData, pieChartDifferencesData, colorPalette, healthCheckCount]);

    return (
        <div className="container mx-auto p-4">
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
                <div className="flex justify-between items-center mb-4">
                    <h2 className="text-xl font-semibold text-gray-800 dark:text-gray-200">HealthCheck Historical Overview</h2>
                    <div className="flex items-center">
                        <label htmlFor="healthCheckCount" className="mr-2 text-gray-700 dark:text-gray-300">Show:</label>
                        <select
                            id="healthCheckCount"
                ...
                <div ref={barChartRef} style={{ height: '300px' }} className="w-full"></div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
                    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-4">
                        <h2 className="text-xl font-semibold mb-4 text-gray-800 dark:text-gray-200">Scripts {selectedHealthCheckNumber} By Type</h2>
                        <div ref={pieChartRef} style={{ height: '300px' }} className="w-full"></div>
                    </div>

                    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-4">
                        <h2 className="text-xl font-semibold mb-4 text-gray-800 dark:text-gray-200">Differences in Script {selectedHealthCheckNumber} By Type</h2>
                        <div ref={pieChartDifferencesRef} style={{ height: '300px' }} className="w-full"></div>
                    </div>
                </div>
            </div>
            ...
            <div className="mt-6 bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
                <div className="mb-4 border-b-2 border-gray-300 dark:border-gray-600">
                    <div
                        className={`inline-block py-2 px-4 cursor-pointer ${activeTab === 'dataTable' ? 'font-semibold text-sky-500' : 'text-gray-500 dark:text-gray-300'}`}
                        onClick={() => setActiveTab('dataTable')}
                    >
                        Data Table
                    </div>
                    <div
                        className={`inline-block py-2 px-4 cursor-pointer ml-4 ${activeTab === 'differencesDataTable' ? 'font-semibold text-sky-500' : 'text-gray-500 dark:text-gray-300'}`}
                ...
                {activeTab === 'dataTable' && (
                    <div>
                        <h2 className="text-xl font-semibold mb-4 text-gray-800 dark:text-gray-200">Data Table</h2>
                        <DataTable data={selectedData} groupBy={groupBy} onGroupByChange={setGroupBy} />
                    </div>
                )}

                {activeTab === 'differencesDataTable' && (
                    <div>
                        <h2 className="text-xl font-semibold mb-4 text-gray-800 dark:text-gray-200">Differences Data Table</h2>
                        <DataTable data={selectedDifferencesData} groupBy={groupBy} onGroupByChange={setGroupBy} />
                    </div>
                )}
            </div>
        </div>
