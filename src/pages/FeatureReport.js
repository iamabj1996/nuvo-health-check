...
import { FiChevronRight, FiChevronDown, FiSearch, FiExternalLink } from 'react-icons/fi';
...
const FeatureReport = () => {
    ...
    const getGroupStatus = (values) => {
        ...
        if (allZero) return { status: 'Inactive', color: 'bg-red-200 text-red-800 dark:bg-red-800 dark:text-red-200' };
        if (allNonZero) return { status: 'Active', color: 'bg-green-200 text-green-800 dark:bg-green-800 dark:text-green-200' };
        return { status: 'Partial', color: 'bg-yellow-200 text-yellow-800 dark:bg-yellow-800 dark:text-yellow-200' };
    };
                    ...
                    <div className="max-h-[calc(100vh-300px)] overflow-y-auto">
                        <table className="min-w-full bg-white dark:bg-gray-800 shadow-md rounded-lg overflow-hidden">
                            <thead className="bg-gray-200 dark:bg-gray-700">
                                <tr>
                                    <th className="px-4 py-2 text-left text-gray-700 dark:text-gray-300">Feature Group</th>
                                    <th className="px-4 py-2 text-left text-gray-700 dark:text-gray-300">Status</th>
                                    <th className="px-4 py-2 text-left text-gray-700 dark:text-gray-300">Current HealthCheck</th>
                                    <th className="px-4 py-2 text-left text-gray-700 dark:text-gray-300">Previous HealthCheck</th>
                                </tr>
                            </thead>
                            <tbody>
                                {Object.entries(filteredData).map(([group, values]) => {
                                    ...
                                    return (
                                        <React.Fragment key={group}>
                                            <tr className="bg-gray-100 dark:bg-gray-600 cursor-pointer" onClick={() => toggleGroup(group)}>
                                                ...
                                                <td className={'px-4 py-1'}>
                                                    <span className={'rounded-full text-xs font-bold px-2 py-1' + color}>
                                                        {status}
                                                    </span>
                                                </td>
                                            ...
                                            {expandedGroups.has(group) && Object.entries(values).map(([subKey, data]) => (

                                                <tr key={subKey} className="border-t border-gray-200 dark:border-gray-700">
                                                    <td className="px-8 py-2 text-gray-800 dark:text-gray-200">
                                                        <a href={data.url} target="_blank" rel="noopener noreferrer" className="flex items-center">
                                                            {subKey}
                                                    ...
                                                    <td className='text-center' > <span style={{ marginRight: '4rem' }} className={
                                                    ...
                                                    <td className="px-4 py-2 text-gray-800 dark:text-gray-200 text-center">
                                                        <span style={{ marginRight: '4rem' }} className={
                                                            'text-sm font-medium px-2 py-1 rounded-full min-w-[40px] text-center ' +
                                                            (Number(data.pastCount) <= Number(data.threshold)
                                                                ? 'bg-red-200 text-red-800 dark:bg-red-800 dark:text-red-200'
                                                                : 'bg-green-200 text-green-800 dark:bg-green-800 dark:text-green-200')
                                                        }> {data.pastCount ? data.pastCount : '-'}</span>
                                                    </td>
                                                </tr>
                                            ))}
                                        </React.Fragment>
